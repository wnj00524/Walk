...
| D17 | Select a pure-GDScript chunked heightmap as the recommended alternative terrain backend; Terrain3D rejected (bounded ~65 km² world, no native runtime procedural streaming); VoxelGraph offset approach documented as fallback only | Coordinator investigation 2026-09-17 evaluated three options: (A) VoxelGraphGenerator offset parameter — possible but unproven, stutter on chunk invalidation, accumulated float precision risk; (B/D) Terrain3D v1.0.2 — world bounded at ~65 km², no built-in runtime noise generator, requires custom streaming; (C) GDScript chunked heightmap — logical 64-bit integer noise inputs immune to float origin shift, no new GDExtension, C03 contract maps directly | T012a bounded spike must prove Option C before changing C03 or setting backend_gate true; if Option C fails performance after one tuning attempt, Option A (VoxelGraph + `set_parameter` + `restart_stream`) is the documented fallback requiring its own bounded spike |
