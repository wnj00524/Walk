...
| game/src/app/main.gd | Loads the saved walker and the validated landform recipe before starting ChunkTerrain | Keep save/resume ordering and recipe failure visible; recipe values are metres/cycles per metre | `python tools/check_game.py --suite landform` plus graphical walk check |
...
| docs/evidence/T024.md | Records the owner-facing visual-quality decision for the generated slice and its missing evidence | Keep ACCEPT/REVISE/NOT REVIEWED decisions tied to retained captures | Owner review gate |
...
| game/data/visual_preset.json | Stores the reviewed terrain palette and fixed daylight controls in one readable record | Keep colour components in 0–1 and energy/scale values within the documented ranges | `python tools/check_game.py --suite visual_preset` |
| game/src/presentation/environment_setup.gd | Validates the visual preset and creates the standard terrain material, world environment, and sun | Change only bounded preset interpretation; preserve explicit failure for invalid values | `python tools/check_game.py --suite visual_preset` |
| game/tests/test_visual_preset.gd | Checks invalid visual values fail and valid values reach the created Godot objects | Add independent malformed presets; do not derive expected values from the object under test | `python tools/check_game.py --suite visual_preset` |
...
| game/src/world/placement_rules.gd | Creates deterministic C04 plant/rock records after actual terrain and approved-asset checks | Density is objects/cell, slopes are rise/run, scale is a positive multiplier, and local axes are half-open metres | `python tools/check_game.py --suite placement` |
| game/data/habitats.json | Holds the reviewed forest, meadow, and rocky grouping budgets consumed by placement generation | Density and cluster counts are bounded visual budgets; preserve distinct habitat ranges | `python tools/check_game.py --suite placement` |
| game/src/presentation/vegetation_batches.gd | Keeps nearby approved placement records in bounded cell batches and instantiates them on the main thread | Resident batches are capped at nine; asset paths must be approved local resources; stale epochs are ignored | `python tools/check_game.py --suite vegetation` |
