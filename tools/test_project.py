...
class ProjectToolTests(unittest.TestCase):
    ...
    def test_ready_brief_contains_rules_and_one_card(self) -> None:
        ...
        brief = project.make_brief(self.root, 'T010')
        self.assertIn('Repository rules for coding agents', brief)
        self.assertIn('T010 - Probe large-distance identity and coordinate handling', brief)
    ...
    def test_waiting_brief_is_refused(self) -> None:
        ...
        with self.assertRaisesRegex(project.PlanError, 'not READY/ACTIVE'):
            project.make_brief(self.root, 'T011')
    ...
    def test_waiting_task_can_be_inspected(self) -> None:
        ...
        self.assertIn('INSPECTION ONLY - DO NOT IMPLEMENT',
                      project.make_brief(self.root, 'T011', inspect=True))
    ...
    def test_premature_ready_task_is_rejected(self) -> None:
        ...
        self.edit('PLAN.md', '| Freeze the terrain adapter contract from probe evidence | WAITING |',
                  '| Freeze the terrain adapter contract from probe evidence | READY |')
        self.assertTrue(any('T011: dependency T010 is not DONE' in e
    ...
    def test_brief_is_not_silently_truncated(self) -> None:
        ...
        with self.assertRaisesRegex(project.PlanError, 'above limit'):
            project.make_brief(self.root, 'T010', max_bytes=2048)
    ...
    def test_cli_failure_is_nonzero(self) -> None:
        ...
        with redirect_stderr(output):
            code = project.main(['--root', str(self.root), 'brief', 'T011'])
